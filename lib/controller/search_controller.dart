...
class SearchController extends GetxController implements GetxService {
  ...
  void setSearchMode(bool isSearchMode) {
    ...
    if (isSearchMode) {
      ...
      _upperValue = 0;
      _lowerValue = 0;
      _searchItemList = Get.find<CategoryController>().categoryItemList;
    }
    update();
  }

  set searchItemList(List<Item>? searchItemList) {
  ...
  void sortItemSearchList(
      {String? type, String? value, bool productPage = false}) {
    _searchItemList = [];
    if (productPage) {
      _searchItemList!.addAll(Get.find<CategoryController>().categoryItemList!);
    } else {
      _searchItemList!.addAll(_allItemList!);
    }
    if (type != null) {
      if (type == 'country') {
        _searchItemList!.removeWhere((product) => product.country != value);
      } else if (type == 'unit') {
        _searchItemList!.removeWhere((product) => product.unitType != value);
      } else if (type == 'vintage') {
        _searchItemList!.removeWhere((product) => product.vintage != value);
      } else if (type == 'item_type') {
        _searchItemList!.removeWhere((product) => product.itemtype != value);
      }
      // else if (type == 'brand') {
      //   _searchItemList!.removeWhere((product) => product.brand != value);
      // }
    } else {
      if (_upperValue > 0) {
      ...
      if (_sortIndex != -1) {
        if (_sortIndex == 0) {
          ...
          // price high to low
          _searchItemList!.sort((a, b) => b.price!.compareTo(a.price!));
        } else {
          // price low to high
          _searchItemList!.sort((a, b) => a.price!.compareTo(b.price!));
        }
      }
    }
    update();
  ...
  getFilterConfig() async {
    Response response = await searchRepo.getFilterConfig();
    if (response.statusCode == 200) {
      _filterConfig = FilterConfig.fromJson(response.body);
    } else {
      ApiChecker.checkApi(response);
    }
    update();
